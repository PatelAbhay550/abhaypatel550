...
- **Interactive sparrow** sitting on a log at the bottom-right of the home page — clicking it plays a sparrow chirp (`/sparrow.mp3`) and tips the bird. (Sparrow conservation nod; sound credit is a Pixabay user, see the FAQ.)
...
- **Paper-texture background** — warm paper tone + subtle SVG grain overlay in `app/globals.css` (darker "charcoal paper" in dark mode), applied site-wide.
...
app/
  ...
  page.tsx            # Home page — renders <FullHome/> (server component)
  ...
  tech-stack/page.tsx # /tech-stack page: title + <SpotlightCard/>
  components/
...
- **Reduced motion**: `globals.css` kills animations/transitions globally for `prefers-reduced-motion`. Don't re-enable motion unconditionally in new components.
...
- **`typedRoutes`** is enabled (top-level) in `next.config.ts` — keep route strings valid.
